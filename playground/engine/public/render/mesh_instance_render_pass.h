...
namespace Render
{
	...
	// Contains everything required to draw a set of Mesh instances in one pass
	class MeshInstanceRenderPass : public RenderPass
	{
	public:
		MeshInstanceRenderPass();
		virtual ~MeshInstanceRenderPass();
		...
		virtual void Reset();
		virtual void RenderAll(Device& device);
